...
如果沒有特別調整，每個角色開始時有四種形象。如果你不希望你的角色太單薄，盡量不要讓形象互相重複：

- **高概念**：角色的壟統的描述。大概就是酒館的人談到角色的時候會使用的介紹詞。
- **麻煩**：讓角色的生活變得複雜的東西。可能是個人弱點、家族糾纏或其他義務。
...
- **重振點**：每個角色都有 3 點**重振點**，這代表每次聚會開始時，這個角色**至少擁有**的**命運點**數量。
...
|卓絕（+4）| [1][1][1][1][1][1] 以及一個額外的**輕微後果格** |
